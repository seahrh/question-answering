...
def run(dataset, preds, OPTS=None):
    if OPTS and OPTS.na_prob_file:
    ...
    na_prob_thresh = 1 if OPTS is None else OPTS.na_prob_thresh
    ...
    if OPTS and OPTS.na_prob_file and OPTS.out_image_dir:
...
def main(OPTS):
    ...
    with open(OPTS.pred_file) as f:
        preds = json.load(f)
    out_eval = run(dataset, preds, OPTS)
    if OPTS.out_file:
